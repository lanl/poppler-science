...
public:
    ...
    void setBBox(const double &m_xMinA, const double &m_yMinA, const double &m_xMaxA, const double &m_yMaxA)
    {
        xMin = m_xMinA;
        yMin = m_yMinA;
        xMax = m_xMaxA;
    ...
    void append(TextWord* m_rhs)
    {
        if(m_rhs == NULL){
            throw __FILE__ ":TextWord::append: Invalid m_rhs == NULL";
        }

        if(rot != m_rhs->rot){
            throw __FILE__ ":TextWord::append: Cannot merge words with different rotations";
        }

        // Append  m_rhs to this word
        for(std::deque<TextWord::CharInfo>::const_iterator i = m_rhs->chars.begin();i != m_rhs->chars.end();++i){
            chars.push_back(*i);
        }
        ...
        switch(rot){
            ...
            case 3:
                ...
                break;
            default:
                throw __FILE__ ":TextWord::append: Invalid rotation";
        };
    };
    ...
    void rotate_counter_clockwise()
    {
        ...
        switch(rot){
            ...
            default:
                ...
                throw __FILE__ ":rotate_counter_clockwise: Invalid rotation";
        };
...
class WordCluster : public std::deque<TextWord*>
{
    public:
        typedef enum {
            TEXT,               // Narrative text
        ...
        double distance2(const WordCluster& m_rhs, const double &m_threshold_primary /*X for rot 0*/, const double &m_threshold_secondary /*Y for rot 0*/) const
        {
            ...
            for(std::deque<TextWord*>::const_iterator iter_i = begin();iter_i != end();++iter_i){

                double xMin, xMax, yMin, yMax;

                #define PACK_BBOX(VECT, WORD_PTR) \
                    (WORD_PTR)->getBBox(&xMin, &yMin, &xMax, &yMax); \
                    VECT[0] = std::make_pair(xMin, yMin); \
                    VECT[1] = std::make_pair(xMin, yMax); \
                    VECT[2] = std::make_pair(xMax, yMin); \
                    VECT[3] = std::make_pair(xMax, yMax);

                std::vector< std::pair<double, double> > bbox_i(4);

                PACK_BBOX(bbox_i, *iter_i);

                for(std::deque<TextWord*>::const_iterator iter_j = m_rhs.begin();iter_j != m_rhs.end();++iter_j){
        ...
        void getBBox(double &m_minX, double &m_minY, double &m_maxX, double &m_maxY) const
        {
            ...
            for(std::deque<TextWord*>::const_iterator i = begin();i != end();++i){
                ...
                m_minX = std::min(m_minX, minX);
                m_minY = std::min(m_minY, minY);
    ...
    void delete_invalid_words()
    {
        for(int rot = 0;rot < 4;++rot){
            ...
            for(int baseIdx = pool->minBaseIdx; baseIdx <= pool->maxBaseIdx;++baseIdx){
                ...
                // Remove any deleted words
                TextWord *prev_w = NULL;
                TextWord *w = pool->getPool(baseIdx);

                while(w != NULL){
                    
                    TextWord *next_w = w->next;
                    ...
                    if(w->valid == false){
                        ...
                        if(w == pool->pool[baseIdx - pool->minBaseIdx].head){
                            pool->pool[baseIdx - pool->minBaseIdx].head = next_w;
                        }
...
public:
    ...
    bool getphysLayout() const { return physLayout; };
    double getfixedPitch() const { return fixedPitch; };
    bool getdoHTML() const { return doHTML; };
...
class POPPLER_PRIVATE_EXPORT TextOutputDevOCR: public TextOutputDev {
public:
  ...
  TextOutputDevOCR(const char *fileName, bool physLayoutA, double fixedPitchA, bool rawOrderA, bool append, bool discardDiagA = false) :
    TextOutputDev(fileName, physLayoutA, fixedPitchA, rawOrderA, append, discardDiagA), splash(NULL), doc(NULL)
    {
        display_header = true;
  ...
  // Create a TextOutputDevOCR which will write to a generic stream.  If
  // <physLayoutA> is true, the original physical layout of the text
  // is maintained.  If <rawOrder> is true, the text is kept in
  // content stream order.
  TextOutputDevOCR(TextOutputFunc func, void *stream, bool physLayoutA, double fixedPitchA, bool rawOrderA, bool discardDiagA = false) :
    TextOutputDev(func, stream, physLayoutA, fixedPitchA, rawOrderA, discardDiagA), splash(NULL), doc(NULL)
  {
    display_header = true;
    display_left_margin = true;
    display_right_margin = true;
