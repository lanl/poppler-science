...
TextWord::TextWord(const GfxState *state, int rotA, double fontSizeA)
{
    ...
    valid = true; // TextWords are valid until marked for deletion 
...
void TextPage::coalesce(bool physLayout, double fixedPitch, bool doHTML, double minColSpacing1)
{
  ...
  for (rot = 0; rot < 4; ++rot) {
	...
	printf("    word: x=%.2f..%.2f y=%.2f..%.2f base=%.2f fontSize=%.2f rot=%d link=%p baseIdx=%d '",
	       word0->xMin, word0->xMax, word0->yMin, word0->yMax,
	       word0->base, word0->fontSize, rot*90, word0->link, baseIdx);
	for (int i = 0; i < word0->getLength(); ++i) {
	  fputc( (* word0->getChar(i) ) & 0xff, stdout);
	}
	printf("'\n");
      }
    }
...
{
    inline bool operator()(const tuple<double /*distance*/, TextWord * /*target*/, TextWord * /*script*/, ScriptLocation> &m_a,
        const tuple<double /*distance*/, TextWord * /*target*/, TextWord * /*script*/, ScriptLocation> &m_b) const
    {
...
void TextPage::coalesce_superscript_and_subscript()
{
    if(pageWidth <= 0.0){
        cerr << "TextPage::coalesce_superscript_and_subscript: Invalid pageWidth" << endl;
        return;
    }

    if(pageHeight <= 0.0){
        cerr << "TextPage::coalesce_superscript_and_subscript: Invalid pageHeight" << endl;
        return;
    }
    ...
    // The page layout in rotation 0 has increasing x from left to right and increasing y from top to bottom
    // text rotation = rot*90 degrees clockwise
    for(int rot = 0;rot < 4; ++rot){

        std::unique_ptr<TextPool> &pool = pools[rot];

         // For each rotation (which indicates a multiple of 90 degrees), bin the words on the page into a grid for fast seaching
        vector< vector< deque<TextWord *> > > grid( SEARCH_GRID_DIM_X, vector< deque<TextWord *> >(SEARCH_GRID_DIM_Y) );

        // Store all potential target + superscript and target + subscript matches so we can sort by match distance
        deque< tuple<double /*distance*/, TextWord * /*target*/, TextWord * /*script*/, ScriptLocation> > candidates;

        for(int baseIdx = pool->minBaseIdx; baseIdx <= pool->maxBaseIdx;++baseIdx){

            for(TextWord *w = pool->getPool(baseIdx);w;w = w->next) {
        ...
        // grid elements.
        unordered_map< TextWord *, unordered_set<TextWord *> > script_count;
        ...
        for(int grid_x = 0;grid_x < SEARCH_GRID_DIM_X;++grid_x){
            for(int grid_y = 0;grid_y < SEARCH_GRID_DIM_Y;++grid_y){
                ...
                const deque<TextWord *>::iterator begin_target = grid[grid_x][grid_y].begin();
                const deque<TextWord *>::iterator end_target = grid[grid_x][grid_y].end();

                for(deque<TextWord *>::iterator iter_target = begin_target;iter_target != end_target;++iter_target){
                    ...
                    PDFRectangle target_box;

                    (*iter_target)->getBBox(&target_box.x1, &target_box.y1, &target_box.x2, &target_box.y2);

                    // Undo the target word rotation
                    if(rot != 0){
                        target_box = target_box.rotate_counter_clockwise(rot);
                    }

                    // Store the best subscript and superscript candidates here
                    TextWord *left_subscript_ptr = NULL;
                    double left_subscript_distance = std::numeric_limits<double>::max();

                    TextWord *left_superscript_ptr = NULL;
                    double left_superscript_distance = std::numeric_limits<double>::max();

                    TextWord *right_subscript_ptr = NULL;
                    double right_subscript_distance = std::numeric_limits<double>::max();

                    TextWord *right_superscript_ptr = NULL;
                    ...
                    for(int grid_offset_x = -1;grid_offset_x <= 1;++grid_offset_x){
                        ...
                        for(int grid_offset_y = -1;grid_offset_y <= 1;++grid_offset_y){
                            ...
                            const deque<TextWord *>::iterator begin_script = grid[grid_x + grid_offset_x][grid_y + grid_offset_y].begin();
                            const deque<TextWord *>::iterator end_script = grid[grid_x + grid_offset_x][grid_y + grid_offset_y].end();

                            for(deque<TextWord *>::iterator iter_script = begin_script;iter_script != end_script;++iter_script){
                                ...
                                PDFRectangle script_box;

                                (*iter_script)->getBBox(&script_box.x1, &script_box.y1, &script_box.x2, &script_box.y2);

                                // Undo the script word rotation
                                if(rot != 0){
                                    script_box = script_box.rotate_counter_clockwise(rot);
                                }
        ...
        for( deque< tuple<double /*distance*/, TextWord * /*target*/, TextWord * /*script*/, ScriptLocation> >::iterator iter = candidates.begin();iter != candidates.end();++iter){

            TextWord *target_ptr = get<1>(*iter);
            TextWord *script_ptr = get<2>(*iter);
            ...
            unordered_map< TextWord *, unordered_set<TextWord *> >::const_iterator count_iter = script_count.find(target_ptr);
            ...
            TextWord::CharInfo reference_char = target_ptr->chars.front();
            ...
            switch( get<3>(*iter) ){
                case LEFT_SUBSCRIPT:
                    {
                        ...
                        for(string::const_reverse_iterator i = subscript_suffix.rbegin();i != subscript_suffix.rend();++i){
                            ...
                            reference_char.text = *i;
                            reference_char.charcode = *i;
                            target_ptr->chars.push_front(reference_char);
                        }

                        for(std::deque<TextWord::CharInfo>::const_reverse_iterator i = script_ptr->chars.rbegin();i != script_ptr->chars.rend();++i){

                            reference_char.text = i->text;
                            reference_char.charcode = i->charcode;
                            target_ptr->chars.push_front(reference_char);
                        }
                        ...
                        switch(rot){
                            ...
                            case 3:
                                ...
                                break;
                            default:
                                throw __FILE__ ":TextPage::coalesce_superscript_and_subscript: Invalid rotation";
                        };
                    }
                    break;
                ...
                case RIGHT_SUBSCRIPT:
                    {
                        ...
                        for(string::const_iterator i = subscript_prefix.begin();i != subscript_prefix.end();++i){
                            ...
                            reference_char.text = *i;
                            reference_char.charcode = *i;
                            target_ptr->chars.push_back(reference_char);
                        }

                        for(std::deque<TextWord::CharInfo>::const_iterator i = script_ptr->chars.begin();i != script_ptr->chars.end();++i){
                ...
                case RIGHT_SUPERSCRIPT:
                    ...
                    break;
                default:
                    throw __FILE__ ":TextPage::coalesce_superscript_and_subscript: Unknown script location";
            };
        }
    }
...
struct SortClustersInReadingOrder
{
    inline bool operator()(const WordCluster &m_a, const WordCluster &m_b) const
    {
        ...
        double minX_a, minY_a, maxX_a, maxY_a;
        double minX_b, minY_b, maxX_b, maxY_b;

        m_a.getBBox(minX_a, minY_a, maxX_a, maxY_a);
        m_b.getBBox(minX_b, minY_b, maxX_b, maxY_b);

        switch( m_a.getRotation() ){
            case 0:
                // Do the clusters overlap in X?
                if( (minX_a > maxX_b) || (minX_b > maxX_a) ){ // No overlap in X
            ...
            case 1:
                ...
                if( (minY_a > maxY_b) || (minY_b > maxY_a) ){ // No overlap in Y
                    return (minY_a < minY_b);
                }
                else{ // Yes overlap in Y
                    return (maxX_a > maxX_b);
                }
                break;
            case 2:
                // Do the clusters overlap in X?
            ...
            case 3:
                ...
                break;
            default:
                throw __FILE__ ":SortClustersInReadingOrder: Invalid rotation";
        };
...
{
    inline bool operator()(TextWord* m_a, TextWord* m_b) const
    {
        if(m_a->getBaseline() == m_b->getBaseline()){
            ...
            switch( m_a->getRotation() ){
                ...
                case 3:
                    return (maxY_a > maxY_b);
                default:
                    throw __FILE__ ":SortWordsInReadingOrder: Invalid rotation";
            };
    ...
    inline bool operator()(TextWord* m_a, TextWord* m_b) const
    {
        ...
        switch( m_a->getRotation() ){
            ...
            case 3:
                ...
                return minX_a < minX_b;
            default:
                throw __FILE__ ":SortWordsByBBox: Invalid rotation";
        };
...
void TextPage::cluster_words()
{
    ...
    for(int rot = 0;rot < 4; ++rot){
        ...
        // Collect all of the words on the page with the given rotation
        std::unique_ptr<TextPool> &pool = pools[rot];

        // Initially, each word defines a cluster
        for(int baseIdx = pool->minBaseIdx; baseIdx <= pool->maxBaseIdx;++baseIdx){
            for(TextWord *w = pool->getPool(baseIdx);w;w = w->next){
    ...
    for(size_t i = 0;i < clusters.size();++i){
        ...
        clusters[i].getBBox(minX, minY, maxX, maxY);

        if(maxY <= header_max_threshold){ // The entire cluster must be above (less than) the header threshold
            header_cluster_indicies.push_back(i);
        }

        if(maxX <= left_margin_max_threshold){ // The entire cluster must be to the left of (less than) than the left margin threshold
            left_margin_cluster_indicies.push_back(i);
        }

        if(minY >= footer_min_threshold){ // The entire cluster must be below (greater than) the footer threshold 
            footer_cluster_indicies.push_back(i);
        }

        if(minX >= right_margin_min_threshold){ // The entire cluster must be to the right of (greater than) than the right margin threshold
    ...
    for(deque<WordCluster>::iterator i = clusters.begin();i != clusters.end();++i){
        ...
        const double word_area = i->word_area();
        const double word_density = (area > 0.0) ? word_area/area : 0.0;
        const size_t num_lines = i->num_line();
        const size_t num_words = i->size();
        const double words_per_line = double(num_words)/num_lines;

        double minX, minY, maxX, maxY;

        i->getBBox(minX, minY, maxX, maxY);

        const double width = (maxX - minX);
        const double height = (maxY - minY);
        const double aspect_ration = (height > 0.0) ? width/height : 0.0;
    ...
    while(true){
        ...
        for(size_t i = 0;i < num_clusters;++i){
            ...
            for(size_t j = i + 1;j < num_clusters;++j){
                ...
                const double d2 = clusters[i].distance2(clusters[j], primary_threshold, secondary_threshold);

                cluster_separation.push_back( make_tuple(d2, i, j) );
            }
        }
        ...
        while( !cluster_separation.empty() ){
            ...
            double trial_minX, trial_minY, trial_maxX, trial_maxY;

            trial.getBBox(trial_minX, trial_minY, trial_maxX, trial_maxY);
            ...
            for(deque<WordCluster>::const_iterator i = clusters.begin();i != clusters.end();++i){
                ...
                if( (minX > trial_maxX) || (trial_minX > maxX) ||
                    (minY > trial_maxY) || (trial_minY > maxY) ){
                    continue;
                }
    ...
    for(deque<WordCluster>::const_iterator i = clusters.begin();i != clusters.end();++i){
        ...
        switch(i->getCategory()){
            ...
            case WordCluster::UNKNOWN:
                ...
                break;
            default:
                throw __FILE__ ": Unknown cluster label!!";
        };
        ...
        cout << "number of words = " << i->size() << endl;
        cout << "number of lines = " << i->num_line() << endl;
        cout << "<words/line> = " << double( i->size() )/i->num_line() << endl;

        double minX, minY, maxX, maxY;

        i->getBBox(minX, minY, maxX, maxY);

        cout << "width = " << (maxX - minX) << endl;
        cout << "height = " << (maxY - minY) << endl;
    ...
    bool m_display_header, bool m_display_left_margin, bool m_display_right_margin, bool m_display_footer)
{
    ...
    for(std::deque<WordCluster>::iterator i = clusters.begin();i != clusters.end();++i){
        ...
        for(WordCluster::iterator j = i->begin();j != i->end();++j){
            ...
            //if( ( (*j) == NULL ) || ((*j)->is_valid() == false) ){
            //    cerr << "Attempting to rotate invalid word " << std::hex << (*j) << std::dec << "; " << (*j)->getText()->c_str() << endl;
            //}

            (*j)->rotate_counter_clockwise(); // Undo any textword rotation
        }
        ...
        switch( i->getCategory() ){
            ...
            case WordCluster::REFERENCE:
                {
                    ...
                    double prev_xMin, prev_yMin, prev_xMax, prev_Ymax;
                    
                    for(WordCluster::const_iterator j = i->begin();j != i->end();++j){

                        double xMin, yMin, xMax, yMax;

                        (*j)->getBBox(&xMin, &yMin, &xMax, &yMax);
                        ...
                        if( (*j)->getBaseline() != prev_baseline ){
                            ...
                            if( (xMin - prev_xMax) > minWordSpacing * (*j)->fontSize ){
            ...
            case WordCluster::UNKNOWN:
                {
                    ...
                    double cluster_xMin, cluster_yMin, cluster_xMax, cluster_yMax;
                    
                    i->getBBox(cluster_xMin, cluster_yMin, cluster_xMax, cluster_yMax);

                    double starting_edge = cluster_xMin; // Justify text from the left edge

                    // Store the coordinate-to-column to maintain left-right table justification
                    ...
                    while( j != i->end() ){
                        ...
                        double first_xMin, first_yMin, first_xMax, first_yMax;

                        (*j)->getBBox(&first_xMin, &first_yMin, &first_xMax, &first_yMax);

                        double last_xMin = first_xMin;
                        double last_yMin = first_yMin;
                        double last_xMax = first_xMax;
                        double last_yMax = first_yMax;
                        ...
                        if( (*j)->getBaseline() != prev_baseline ){
                            ...
                            prev_baseline = (*j)->getBaseline();
                            starting_edge = cluster_xMin; // Justify text from the left edge
                            (*outputFunc)(outputStream, eol, eolLen);
                            col = 0;
                        ...
                        while( next_word != i->end() ){
                            ...
                            double next_xMin, next_yMin, next_xMax, next_yMax;

                            (*next_word)->getBBox(&next_xMin, &next_yMin, &next_xMax, &next_yMax);

                            const double min_font_size = std::min( (*last_word)->fontSize, (*next_word)->fontSize );

                            if( (next_xMin - last_xMax) <= maxWordSpacing * min_font_size ){
                                
                                if( (next_xMin - last_xMax) > minWordSpacing * min_font_size ){
                        ...
                        //    cout << (*k)->getText()->c_str();
                        //}
                        //cerr << ": xMin=" << first_xMin<< ";yMin=" << first_yMin << ";xMax=" << last_xMax << ";yMax=" << last_yMax 
                        //    << ";baseline=" << (*j)->getBaseline() << ";min word space=" << minWordSpacing * (*j)->fontSize << endl;
                        ...
                        deque< pair<double, int> >::const_iterator iter = 
                            find_closest_col(coord_to_col.begin(), coord_to_col.end(), first_xMin, maxWordSpacing * (*first_word)->fontSize);
                        ...
                        if( iter != coord_to_col.end() ){ // Left justify
                            ...
                            iter = find_closest_col(coord_to_col.begin(), coord_to_col.end(), last_xMax, maxWordSpacing * (*last_word)->fontSize);
                            ...
                            if(iter != coord_to_col.end() ){ // Right justify
                                ...
                                iter = find_closest_col(coord_to_col.begin(), coord_to_col.end(), 0.5*(last_xMax + last_xMin), maxWordSpacing * (*last_word)->fontSize);
                                ...
                                if(iter != coord_to_col.end() ){ // Center justify
                                    ...
                                    // Approximate spacing
                                    int approx_col = interpolate_col(coord_to_col.begin(), coord_to_col.end(), first_xMin);
                                    ...
                                    if(approx_col >= 0){ // Approximate left justification
                                        ...
                                        approx_col = interpolate_col(coord_to_col.begin(), coord_to_col.end(), last_xMax);
                                        ...
                                        if(approx_col >= 0){ // Approximate right justification
                                            ...
                                            const double delta_space = first_xMin - starting_edge;
                                            const double ave_char_width = first_to_last_col ? (last_xMax - first_xMin)/first_to_last_col : 0.0;

                                            const int num_space = (ave_char_width > 0.0) ? round(delta_space/ave_char_width) : 0;

                                            // DEBUG
                                            //cerr << "\tApprox " << num_space << ";ave_char_width = " << ave_char_width << ";delta space = " << delta_space << endl;
                        ...
                        // DEBUG
                        //cerr << "\tfirst xMin (" << first_xMin << ") mapped to column " << col << endl;

                        coord_to_col.push_back( make_pair(first_xMin, col) ); // position of first character
                        ...
                        starting_edge = last_xMax;

                        if( (col - begin_col) > 2){
                            
                            coord_to_col.push_back( make_pair( 0.5*(first_xMin + last_xMax), (col - 1 + begin_col)/2) ); // position of center character
                            //cerr << "\tmid X (" << 0.5*(first_xMin + last_xMax) << ") mapped to column " << (col - 1 + begin_col)/2 << endl;

                            coord_to_col.push_back( make_pair(last_xMax, col - 1) ); // position of last character
            ...
            case WordCluster::HEADER:
                if(m_display_header){
                    ...
                    for(WordCluster::const_iterator j = i->begin();j != i->end();++j){
                        ...
                        if(needs_space && ( (xMin - prev_xMax) > minWordSpacing * (*j)->fontSize) ){
                            (*outputFunc)(outputStream, space, spaceLen);
                        }
                    ...
                    //(*outputFunc)(outputStream, eol, eolLen); // DEBUG
                    //(*outputFunc)( outputStream, meta_data_end.c_str(), meta_data_end.size() );
                }
                break;
            case WordCluster::LEFT_MARGIN:
            ...
            case WordCluster::FOOTER:
                ...
                break;
            default:
                throw __FILE__ ":TextPage::dump: Unknown cluster category";
        };
    }
...
void TextOutputDev::endPage()
{
    ...
    if (outputStream) {
        text->dump(outputStream, outputFunc, physLayout, textEOL, textPageBreaks, 
            true /*show header*/, true /*show left margin*/, true /*show right margin*/, true /*show footer*/);
    }
}
...
void TextOutputDevOCR::endPage() // Overloaded to provide page dimensions to TextPage::coalesce()
{
    if(doc == NULL){
        throw __FILE__ ":TextOutputDevOCR::endPage: Invalid doc pointer";
    }

    getText()->endPage();
    getText()->coalesce( getphysLayout(), getfixedPitch(), getdoHTML(), getMinColSpacing1() );

    if( getoutputStream() ) {
        getText()->dump(getoutputStream(), getoutputFunc(), getphysLayout(), gettextEOL(), gettextPageBreaks(),
            display_header, display_left_margin, display_right_margin, display_footer);
    }
}
...
void TextOutputDevOCR::drawChar(GfxState *state, double x, double y,
        ...
        if( glyph_iter != corrected_glyphs.end() ){
            ...
            if(!is_empty_glyph){ // Only output and/or classifiy glyphs that have a least *one* pixel set
                ...
                if(dump_glyphs){
                    ...
                    for(int y = bitmap_height - 1;y >= 0;--y){ // Invert the bitmap for human readability when debugging
                        for(int x = 0;x < bitmap_width;++x){
                            ...
                            printf("%d", int(glyph_bmp.first[y*bitmap_width + x]));
                        }
                ...
                if(infer_glyphs){
                    ...
                    if(replacement_u[unicode_index] != best_unicode){
                        ...
                        // Do not attempt to correct reported glyphs that are too similar to the predicted glyph to allow reliable classification
                        if( (best_p >= prediction_probability_threshold) && (self_p <= self_probability_threshold) ){
  ...
  // Pass the potentially-modified unicode to the base class for writing to the output stream/file
  TextOutputDev::drawChar(state, x, y, dx, dy, originX, originY, c, nBytes, replacement_u, uLen);
  ...
  TextOutputDevOCR::renderGlyphToBitmap(SplashOutputDev *m_splash, GfxState *state, const CharCode c, const double glyph_width, const double glyph_height, const double glyph_pt_size)
{
  ...
  if(doc == NULL){
    throw __FILE__ ":TextOutputDevOCR::renderGlyphToBitmap: doc == NULL";
  }
...
void WordCluster::align_baselines()
{
    ...
    double prev_minX = std::numeric_limits<double>::lowest();
    double prev_minY = std::numeric_limits<double>::lowest();
    double prev_maxX = std::numeric_limits<double>::lowest();
    double prev_maxY = std::numeric_limits<double>::lowest();
    ...
    for(WordCluster::iterator i = begin();i != end();++i){
        ...
        switch(rot){
                ...
                if(prev_maxY <= minY){ // No overlap
                    ...
                    if( (*i)->getBaseline() != prev_baseline ){
                        ...
                        if( fabs(dy) < overlap_threshold * (*i)->getFontSize() ){
                            ...
                            (*i)->setBBox(minX, minY + dy, maxX, maxY + dy);
                ...
                if(prev_maxX <= minX){ // No overlap
                    ...
                    if( (*i)->getBaseline() != prev_baseline ){
                        ...
                        if( fabs(dx) < overlap_threshold * (*i)->getFontSize() ){
                            ...
                            (*i)->setBBox(minX + dx, minY, maxX + dx, maxY);
                            (*i)->setBaseline(prev_baseline);
                        }
                    }
                }
            ...
            case 3:
                ...
                break;
            default:
                throw __FILE__ ":WordCluster::align_baselines: Invalid rotation";
        };
    }
}
...
void WordCluster::merge_word_fragments()
{
    ...
    while( iter != end() ){
        ...
        if( ( (*iter)->getRotation() == (*next_iter)->getRotation() ) &&
            ...
            (*iter)->getBBox(&xMin, &yMin, &xMax, &yMax);
            (*next_iter)->getBBox(&next_xMin, &next_yMin, &next_xMax, &next_yMax);

            const double min_font_size = std::min( (*iter)->getFontSize(), (*next_iter)->getFontSize() );

            switch(rot){
                case 0:
                    if( (next_xMin - xMax) <= minWordSpacing*min_font_size ){
                    ...
                    break;
                case 1:
                    if( (next_yMin - yMax) <= minWordSpacing*min_font_size ){
                    ...
                    break;
                case 2:
                    if( (xMin - next_xMax) <= minWordSpacing*min_font_size ){
                    ...
                    break;
                case 3:
                    if( (yMin - next_yMax) <= minWordSpacing*min_font_size ){
                    ...
                    break;
                default:
                    throw __FILE__ ":WordCluster::merge_word_fragments: Invalid rotatin";
            };
        }
...
double WordCluster::line_multiword_density() const
{
    ...
    for(std::deque<TextWord*>::const_iterator i = begin();i != end();++i){
        ...
        if(local.num_word == 0){
            ...
            local.bbox.x1 = std::min(local.bbox.x1, minX);
            local.bbox.y1 = std::min(local.bbox.y1, minY);
            local.bbox.x2 = std::max(local.bbox.x2, maxX);
            local.bbox.y2 = std::max(local.bbox.y2, maxY);
        }

        ++local.num_word;
        local.word_area += (maxX - minX)*(maxY - minY);
    }
