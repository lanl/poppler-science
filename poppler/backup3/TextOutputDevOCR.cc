...
void TextPageOCR::coalesce_superscript_and_subscript(const bool m_display_subscript, const bool m_display_superscript)
{
    ...
    unordered_map<TextWordOCR* /*target*/, std::deque< std::pair<TextWordOCR*, ScriptLocation> > /*script, location*/> left_matches;
    unordered_map<TextWordOCR* /*target*/, std::deque< std::pair<TextWordOCR*, ScriptLocation> > /*script, location*/> right_matches;
    ...
    while(true){
                        ...
                        for(int grid_offset_y = -1;grid_offset_y <= 1;++grid_offset_y){
                            ...
                            for(deque<TextWordOCR *>::iterator iter_script = begin_script;iter_script != end_script;++iter_script){
                                ...
                                if(m_display_subscript){
                                    ...
                                    target_overlap = max(0.0, target_box.y2 - script_box.y1)/(target_box.y2 - target_box.y1);
                                    ...
                                    if( (dx >= delta.x1) && (dy >= delta.y1) && (dx <= delta.x2) && (dy <= delta.y2) && (target_overlap <= MAX_TARGET_Y_OVERLAP) ){
                                        ...
                                        if(left_subscript_distance > d){
                                            ...
                                            // DEBUG
                                            //cout << "left sub: " << (*iter_script)->getText()->toStr() << "<-" << (*iter_target)->getText()->toStr() 
                                            //    << ";dx = " << dx << ";dy = " << dy 
                                            ...
                                            //    << ";script font = " << (*iter_script)->fontSize << ";target font = " << (*iter_target)->fontSize
                                            //    << ";target_overlap = " << target_overlap << endl;
                                ...
                                if(m_display_superscript){
                                    ...
                                    target_overlap = max(0.0, script_box.y2 - target_box.y1)/(target_box.y2 - target_box.y1);
                                ...
                                if(m_display_subscript){
                                    ...
                                    //cout << "# delta.x1 = " << delta.x1 << ";delta.y1 = " << delta.y1 << ";delta.x2 = " << delta.x2 << ";delta.y2 = " << delta.y2 << endl;
                                    //cout << "right sub: " << (*iter_script)->getText()->toStr() << "<-" << (*iter_target)->getText()->toStr() 
                                    //    << ";dx = " << dx << ";dy = " << dy 
                                    //    << ";script " << script_box.x1 << ".." << script_box.x2 << " " << script_box.y1 << ".." << script_box.y2 
                                    //    << ";target " << target_box.x1 << ".." << target_box.x2 << " " << target_box.y1 << ".." << target_box.y2 
                                    //    << ";script baseline = " << (*iter_script)->getBaseline() << ";target baseline = " << (*iter_target)->getBaseline() 
                                ...
                                if(m_display_superscript){
                                    ...
                                    if( (dx >= delta.x1) && (dy >= delta.y1) && (dx <= delta.x2) && (dy <= delta.y2) && (target_overlap <= MAX_TARGET_Y_OVERLAP) ){
                                        ...
                                        if(right_superscript_distance > d){
                                            ...
                                            // DEBUG
                                            //cout << "# delta.x1 = " << delta.x1 << ";delta.y1 = " << delta.y1 << ";delta.x2 = " << delta.x2 << ";delta.y2 = " << delta.y2 << endl;
                                            //cout << "right super: " << (*iter_script)->getText()->toStr() << "<-" << (*iter_target)->getText()->toStr() 
                                            //    << ";dx = " << dx << ";dy = " << dy 
                                            //    << ";script " << script_box.x1 << ".." << script_box.x2 << " " << script_box.y1 << ".." << script_box.y2 
                                            //    << ";target " << target_box.x1 << ".." << target_box.x2 << " " << target_box.y1 << ".." << target_box.y2 
                                            //    << ";script baseline = " << (*iter_script)->getBaseline() << ";target baseline = " << (*iter_target)->getBaseline() 
        ...
        for( deque< tuple<double /*distance*/, TextWordOCR * /*target*/, TextWordOCR * /*script*/, ScriptLocation> >::iterator iter = candidates.begin();iter != candidates.end();++iter){
            ...
            unordered_set<double> script_support_x;
            unordered_set<double> script_support_y;
            ...
            #define ADD_SUPPORT_X() \
                for(unordered_set<double>::const_iterator i = script_support_x.begin();i != script_support_x.end();++i){ \
            ...
            #define ADD_SUPPORT_Y() \
                for(unordered_set<double>::const_iterator i = script_support_y.begin();i != script_support_y.end();++i){ \
                ...
                case LEFT_SUBSCRIPT:
                    {
                        ...
                        left_matches[target_ptr].push_back( make_pair(script_ptr, get<3>(*iter) ) );
                ...
                case RIGHT_SUBSCRIPT:
                    {
                        ...
                        //    << "; script size = " << script_ptr->fontSize << "; script baseline = " << script_ptr->getBaseline() << "; count = " << count_iter->second.size() 
                        //    << ";target_ptr = " << target_ptr << ";script_ptr = " << script_ptr << endl;
                        ...
                        right_matches[target_ptr].push_back( make_pair(script_ptr, get<3>(*iter) ) );
                ...
                case RIGHT_SUPERSCRIPT:
                    {
                        ...
                        //   << "; target size = " << target_ptr->fontSize << "; script size = " << script_ptr->fontSize << "; count = " << count_iter->second.size()
                        //   << ";target_ptr = " << target_ptr << ";script_ptr = " << script_ptr << endl;
    ...
    for(unordered_map<TextWordOCR* /*target*/, std::deque< std::pair<TextWordOCR*, ScriptLocation> > >::const_iterator i = left_matches.begin();i != left_matches.end();++i){
            ...
            if(last_script_loc != j->second){
                ...
                if(j->second == LEFT_SUBSCRIPT){
                    ...
                    if(last_script_loc != UNKNOWN_SCRIPT){
                        ...
                        for(string::const_reverse_iterator k = superscript_suffix.rbegin();k != superscript_suffix.rend();++k){
                    ...
                    for(string::const_reverse_iterator k = subscript_suffix.rbegin();k != subscript_suffix.rend();++k){
                        i->first->chars.push_front( TextWordOCR::CharInfo( *k, *k, reference_char.edge, reference_char.font, true /*formatting metadata*/) );
                    }
        ...
        if(last_script_loc == LEFT_SUBSCRIPT){
            for(string::const_reverse_iterator j = subscript_prefix.rbegin();j != subscript_prefix.rend();++j){
            ...
            for(string::const_reverse_iterator j = superscript_prefix.rbegin();j != superscript_prefix.rend();++j){
                i->first->chars.push_front( TextWordOCR::CharInfo( *j, *j, reference_char.edge, reference_char.font, true /*formatting metadata*/) );
            }
        }
    }

    for(unordered_map<TextWordOCR* /*target*/, std::deque< std::pair<TextWordOCR*, ScriptLocation> > >::const_iterator i = right_matches.begin();i != right_matches.end();++i){
        ...
        for(std::deque< std::pair<TextWordOCR*, ScriptLocation> >::const_iterator j = i->second.begin();j != i->second.end();++j){
            ...
            if(last_script_loc != j->second){
                ...
                if(j->second == RIGHT_SUBSCRIPT){
                    ...
                    if(last_script_loc != UNKNOWN_SCRIPT){
                        // Write </sup> to close the previous superscript
                        for(string::const_iterator k = superscript_suffix.begin();k != superscript_suffix.end();++k){
                            i->first->chars.push_back( TextWordOCR::CharInfo( *k, *k, reference_char.edge, reference_char.font, true /*formatting metadata*/) );
                        }
                    }

                    // Write <sub> to start the next subscript
                    for(string::const_iterator k = subscript_prefix.begin();k != subscript_prefix.end();++k){
                        i->first->chars.push_back( TextWordOCR::CharInfo( *k, *k, reference_char.edge, reference_char.font, true /*formatting metadata*/) );
                    }
                    ...
                    if(last_script_loc != UNKNOWN_SCRIPT){
                        // Write </sub> to close the previous subscript
                        for(string::const_iterator k = subscript_suffix.begin();k != subscript_suffix.end();++k){
                            i->first->chars.push_back( TextWordOCR::CharInfo( *k, *k, reference_char.edge, reference_char.font, true /*formatting metadata*/) );
                        }
                    }

                    // Write <sup> to start the next superscript
                    for(string::const_iterator k = superscript_prefix.begin();k != superscript_prefix.end();++k){
                        i->first->chars.push_back( TextWordOCR::CharInfo( *k, *k, reference_char.edge, reference_char.font, true /*formatting metadata*/) );
                    }
                }
            }
        ...
        if(last_script_loc == RIGHT_SUBSCRIPT){
            for(string::const_iterator j = subscript_suffix.begin();j != subscript_suffix.end();++j){
            ...
            for(string::const_iterator j = superscript_suffix.begin();j != superscript_suffix.end();++j){
                i->first->chars.push_back( TextWordOCR::CharInfo( *j, *j, reference_char.edge, reference_char.font, true /*formatting metadata*/) );
            }
        }
    }
...
void TextPageOCR::cluster_words()
{
    ...
    vector< tuple<double /*double*/, size_t /*min cluster index*/, size_t /*max cluster index*/> > d2;

    d2.reserve( (num_cluster*(num_cluster - 1) )/2);

    vector<size_t> index(num_cluster); // The current cluster indicies to reflect cluster merging
        ...
        for(size_t j = i + 1;j < num_cluster;++j){
            ...
            if( rot_i == clusters[j].getRotation() ){
                ...
                if( tmp != std::numeric_limits<double>::max() ){
                    d2.push_back( make_tuple(tmp, i, j) );
                }
            }
        }
    }
    ...
    while( !d2.empty() ){ // Iteratively merge clusters using single linkage clustering
        ...
        const tuple<double, size_t, size_t> best = d2.back();
        ...
        // DEBUG
        if(clusters[best_i].empty()){
            cerr << "** Warning ** cluster[" << best_i << "] is empty" << endl;
        }

        if(clusters[best_j].empty()){
            cerr << "** Warning ** cluster[" << best_j << "] is empty" << endl;
        }
    ...
    for(vector<WordCluster>::iterator i = clusters.begin();i != clusters.end();++i){
    ...
    for(size_t i = 0;i < num_cluster;++i){
        ...
        for(size_t j = i + 1;j < num_cluster;++j){
            ...
            d2.push_back( make_tuple(clusters[i].distance2(clusters[j]), i, j) ); // <-- NO cluster separation thresholds when computing d2!
        }
    }
    ...
    while( !d2.empty() ){ // Iteratively merge clusters using single linkage clustering
        ...
        for(vector<WordCluster>::const_iterator i = clusters.begin();i != clusters.end();++i){
...
void TextPageOCR::dump(void *outputStream, TextOutputFunc outputFunc, EndOfLineKind textEOL, const bitset<NUM_OPTIONS> &m_options)
{
    const UnicodeMap *uMap;
        ...
        switch( i->getCategory() ){
            ...
            case WordCluster::HEADER:
                if(m_options[DISPLAY_HEADER]){
                    ...
                    // Metadata
                    const string meta_data_begin = "<header>";
            ...
            case WordCluster::LEFT_MARGIN:
                if(m_options[DISPLAY_LEFT_MARGIN]){
                    ...
                    // Metadata
                    const string meta_data_begin = "<left_margin>";
            ...
            case WordCluster::RIGHT_MARGIN:
                if(m_options[DISPLAY_RIGHT_MARGIN]){
                    ...
                    // Metadata
                    const string meta_data_begin = "<right_margin>";
            ...
            case WordCluster::FOOTER:
                if(m_options[DISPLAY_FOOTER]){
                    ...
                    // Metadata
                    const string meta_data_begin = "<footer>";
...
void TextOutputDevOCR::drawChar(GfxState *state, double x, double y,
        ...
        if(valid_font){
            ...
            for(int unicode_index = 0;unicode_index < uLen;++unicode_index){
                ...
                if( glyph_iter != corrected_glyphs.end() ){
                    ...
                    const pair< vector<unsigned char>, pair<int /*width*/, int /*height*/> > glyph_bmp = 
    ...
    if(options[SPLIT_LIGATURE]){
        ...
        const deque<Unicode> ligature_buffer = split_ligature(replacement_u, uLen);
...
deque<Unicode> TextOutputDevOCR::split_ligature(const Unicode *u, int uLen)
{
