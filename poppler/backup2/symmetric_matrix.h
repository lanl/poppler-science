...
class SymmetricMatrix : private std::vector<T>
{
	...
	public:
		...
		inline T& operator()(size_t m_row /*copy*/, size_t m_col /*copy*/)
		{
			...
			if(max_index >= len){
				error(errInternal, -1, __FILE__ ":SymmetricMatrix::operator(): Index out of bounds");
			}
		...
		inline T operator()(size_t m_row /*copy*/, size_t m_col /*copy*/) const
		{
			...
			if(max_index >= len){
				error(errInternal, -1, __FILE__ ":SymmetricMatrix::operator() const: Index out of bounds");
			}
		...
		{
			if(len != m_rhs.len){
				error(errInternal, -1, __FILE__ ":SymmetricMatrix::operator+=: Cannot combine matricies with unequal sizes");
			}
