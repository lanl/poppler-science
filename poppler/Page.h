...
public:
    ...
    inline double& xMin(){return x1;};
    inline double& yMin(){return y1;};
    inline double& xMax(){return x2;};
    inline double& yMax(){return y2;};

    inline double xMin() const {return x1;};
    inline double yMin() const {return y1;};
    inline double xMax() const {return x2;};
    inline double yMax() const {return y2;};

    inline bool isValid() const { return (x1 != 0) || (y1 != 0) || (x2 != 0) || (y2 != 0); }
    inline bool contains(double x, double y) const { return (x1 <= x) && (x <= x2) && (y1 <= y) && (y <= y2); }
    void clipTo(PDFRectangle *rect);
    PDFRectangle rotate_counter_clockwise(const int m_rot) const;
    inline double area() const { return (x2 - x1)*(y2 - y1);};

    bool operator ==(const PDFRectangle &rect) const { return (x1 == rect.x1) && (y1 == rect.y1) && (x2 == rect.x2) && (y2 == rect.y2); }

    // Expand the rectangle (if needed) to include the input rectange
    inline PDFRectangle& operator |=(const PDFRectangle &m_rhs){

        x1 = std::min(x1, m_rhs.x1);
        y1 = std::min(y1, m_rhs.y1);

        x2 = std::max(x2, m_rhs.x2);
        y2 = std::max(y2, m_rhs.y2);

        return *this;
    };

    inline PDFRectangle operator |(const PDFRectangle &m_rhs) const{
        
        PDFRectangle ret(*this);
