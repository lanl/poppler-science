...
static bool ocrDumpGlyphs = false;
...
static const ArgDesc argDesc[] = { { "-f", argInt, &firstPage, 0, "first page to convert" },
                                   ...
                                   {"-ocr.best_threshold", argFP, &ocrBestThreshold, 0, "minimun threshold for highest probability OCR inferred glyph"},
                                   {"-ocr.self_threshold", argFP, &ocrSelfThreshold, 0, "maximum threshold for probability of reported glyph"},
                                   {"-ocr.dump_glyphs", argFlag, &ocrDumpGlyphs, 0, "write all glyph bitmaps to stdout"},
...
int main(int argc, char *argv[])
{
    ...
    if (htmlMeta && bbox) { // htmlMeta && is superfluous but makes gcc happier
        ...
        if (tsvMode) {
            ...
            //TextOutputDev textOut(nullptr, physLayout, fixedPitch, rawOrder, htmlMeta, discardDiag);
            TextOutputDevOCR textOut(nullptr, physLayout, fixedPitch, rawOrder, htmlMeta, discardDiag);
            ...
            textOut.startDoc(doc.get(), ocrModelFileName, ocrBestThreshold, ocrSelfThreshold, ocrDumpGlyphs);
            ...
            TextOutputDevOCR textOut(textFileName->c_str(), physLayout, fixedPitch, rawOrder, htmlMeta, discardDiag);

            if(noHeader){
                textOut.disable_header();
            }

            if(noLeftMargin){
                textOut.disable_left_margin();
            }

            if(noRightMargin){
                textOut.disable_right_margin();
            }

            if(noFooter){
                textOut.disable_footer();
            }
            ...
            if (textOut.isOk()) {
                ...
                if(ocrDumpGlyphs){
                    // This simple end of document string allows us to measure the *per-document* frequency of a given glyph.
